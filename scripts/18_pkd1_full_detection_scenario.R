# scripts/18_pkd1_full_detection_scenario.R
# Purpose: Re-run the full PSA assigning PKD1 detection = 1.0 in Panel and ES (near-future probe design).
# Author: Zachary Sentell

# Standard short-read WES detects only ~7% of PKD1 variants in exons 1-32 due to competitive
# capture by six pseudogenes sharing ~97.7% identity. Chang et al. (2022) showed that a modified
# xGEN probe design with pseudogene-aware forced-alignment achieves complete coverage with 100%
# clinical concordance. This scenario models that near-future state. A full PSA re-run is
# required because detection changes affect per-proband diagnosis probabilities non-linearly.
